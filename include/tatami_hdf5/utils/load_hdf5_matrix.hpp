...
tatami::CompressedSparseMatrix<ROW, T, IDX, U, V, W> load_hdf5_compressed_sparse_matrix(
    size_t nr, 
    size_t nc, 
    ...
    const std::string& ptr) 
{
    ...
    U x(nonzeros);
    dhandle.read(x.data(), define_mem_type<typename U::value_type>());
    ...
    V i(nonzeros);
    ihandle.read(i.data(), define_mem_type<typename V::value_type>());
    ...
    if constexpr(std::is_same<size_t, typename W::value_type>::value) {
        ...
        phandle.read(p.data(), define_mem_type<typename W::value_type>());
    }

    return tatami::CompressedSparseMatrix<ROW, T, IDX, U, V, W>(nr, nc, std::move(x), std::move(i), std::move(p));
}
...
tatami::DenseMatrix<!transpose, T, IDX, V> load_hdf5_dense_matrix(const std::string& file, const std::string& name) {
    ...
    dhandle.read(values.data(), define_mem_type<typename V::value_type>());

    if constexpr(transpose) {
        return tatami::DenseMatrix<false, T, IDX, V>(dims[1], dims[0], std::move(values));
    } else {
        return tatami::DenseMatrix<true, T, IDX, V>(dims[0], dims[1], std::move(values));
    }
}
